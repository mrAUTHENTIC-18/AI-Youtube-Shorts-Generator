...
def _safe_remove(path: str):
    """Remove file safely on Windows where files can remain temporarily locked."""
    ...
    print(f"[cleanup] could not remove locked file: {path}")
...
def _reframe_vertical(in_path: str, out_path: str, aspect_ratio: str) -> str:
    ...
    while True:
        ...
        x0 = max(
            0,
            min(src_w - crop_w, cx - crop_w // 2)
        )

        y0 = max(
            0,
            min(src_h - crop_h, cy - crop_h // 2)
        )
